...
class MyContactsView: SliderVC {
    ...
    override func viewDidLoad() {
        ...
        actionButton.addItem(title: "", image: #imageLiteral(resourceName: "add_contact")) { item in
            ...
            let vc = storyboard.instantiateViewController(withIdentifier: "idAddContactView") as! AddContactView
...
extension MyContactsView : DAOSearchBarDelegate, UITextFieldDelegate {
...
extension MyContactsView : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction.init(style: UIContextualAction.Style.destructive, title: "Delete", handler: { (action, view, completion) in
            ...
            let alertVC = PMAlertController(title: "", description: "Do you want to delete this Contact?", image: nil, style: .alert)
...
extension MyContactsView {
    ...
    @IBAction func actionAssignCampaignsToAllRecords(_ sender: UIButton) {
        ...
        if arrSelectedRecords.count > 0 && arrSelectedRecords.count <= 50 {
            ...
            OBJCOM.hideLoader()
            let alertVC = PMAlertController(title: "", description: "You can assign email campaign only 50 Contacts. Please select Contacts less than 50.", image: nil, style: .alert)
    ...
    func selectOptionsForMoveProspects(prospectId: String) {
        let item1 = ActionSheetItem(title: "My Customers", value: 1)
        let item2 = ActionSheetItem(title: "My Prospects", value: 2)
        let item3 = ActionSheetItem(title: "My Recruits", value: 3)
...
extension MyContactsView {
    @IBAction func actionMoreOptions(_ sender:AnyObject) {
        let item1 = ActionSheetItem(title: "Import Device Contacts", value: 1)
        let item2 = ActionSheetItem(title: "Import Google Contacts", value: 2)
        let item3 = ActionSheetItem(title: "Import Newly Added Contacts", value: 3)
        let item4 = ActionSheetItem(title: "Import Contacts CSV", value: 4)
        let item5 = ActionSheetItem(title: "Export Contacts", value: 5)
        let item6 = ActionSheetItem(title: "Help", value: 6)
        
        let button = ActionSheetOkButton(title: "Dismiss")
        let items = [item1, item2, item3, item4, item5, item6, button]
        let sheet = ActionSheet(items: items) { sheet, item in
            if item.title != "Dismiss"{
                if item == item1 {
                    ...
                    let storyboard = UIStoryboard(name: "MyContacts", bundle: nil)
                    let vc = storyboard.instantiateViewController(withIdentifier: "idImportGoogleContacts") as! ImportGoogleContacts
                    vc.modalTransitionStyle = .coverVertical
                    vc.modalPresentationStyle = .custom
                    vc.view.backgroundColor = UIColor.black.withAlphaComponent(0.8)
                    self.present(vc, animated: true, completion: nil)
                    ...
                    let storyboard = UIStoryboard(name: "MyProspects", bundle: nil)
                    let vc = storyboard.instantiateViewController(withIdentifier: "idImportNewContacts") as! ImportNewContacts
                    vc.contact_flag = "1"
                    vc.modalTransitionStyle = .coverVertical
                    vc.modalPresentationStyle = .custom
                    vc.view.backgroundColor = UIColor.black.withAlphaComponent(0.8)
                    ...
                    self.createCSV(from: self.prospectCsvArray, crmFlag: "1")
                }else if item == item6 {
                    let storyboard = UIStoryboard(name: "MyContacts", bundle: nil)
                    let vc = storyboard.instantiateViewController(withIdentifier: "idHelpMyContacts") as! HelpMyContacts
                    vc.modalTransitionStyle = .coverVertical
                    vc.modalPresentationStyle = .custom
                    vc.view.backgroundColor = UIColor.darkGray.withAlphaComponent(0.8)
                    self.present(vc, animated: true, completion: nil)
    ...
    func createCSV(from recArray:[Dictionary<String, AnyObject>], crmFlag : String) {
        ...
        do {
            ...
            let fileURL = path.appendingPathComponent("csvContacts.csv")
    ...
    func exportCSVAPI(crmFlag : String){
        ...
        OBJCOM.modalAPICall(Action: "sendCsvbyEmailCrm", param:dictParam as [String : AnyObject],  vcObject: self) {
            ...
            if success == "true"{
                ...
                let alertVC = PMAlertController(title: "", description: "Contacts exported successfully.", image: nil, style: .alert)
...
extension MyContactsView : EPPickerDelegate {
    ...
    func epContactPicker(_: EPContactsPicker, didSelectMultipleContacts contacts: [EPContact]) {
        ...
        for contact in contacts {
            ...
            if contact.emails.count == 1 {
                em = ["em1":contact.emails[0].email,
                      "em2":"",
                      "em3":""]
            }else if contact.emails.count == 2 {
                em = ["em1":contact.emails[0].email,
                      "em2":contact.emails[1].email,
                      "em3":""]
            }else if contact.emails.count >= 3 {
                em = ["em1":contact.emails[0].email,
                      "em2":contact.emails[1].email,
                      "em3":contact.emails[2].email]
            }
            ...
            if contact.phoneNumbers.count == 1 {
                arrPh = ["ph1":contact.phoneNumbers[0].phoneNumber,
                         "ph2":"",
                         "ph3":""]
            }else if contact.phoneNumbers.count == 2 {
                arrPh = ["ph1":contact.phoneNumbers[0].phoneNumber,
                         "ph2":contact.phoneNumbers[1].phoneNumber,
                         "ph3":""]
            }else if contact.phoneNumbers.count >= 3 {
                arrPh = ["ph1":contact.phoneNumbers[0].phoneNumber,
                         "ph2":contact.phoneNumbers[1].phoneNumber,
                         "ph3":contact.phoneNumbers[2].phoneNumber]
            }
            ...
            contactArray["contact_fname"] = contact.firstName
            contactArray["contact_lname"] = contact.lastName
            contactArray["contact_company_name"] = contact.company
            contactArray["contact_email"] = em["em1"]
            contactArray["contact_work_email"] = em["em2"]
            contactArray["contact_other_email"] = em["em3"]
            contactArray["contact_phone"] = arrPh["ph1"]
            contactArray["contact_work_phone"] = arrPh["ph2"]
            contactArray["contact_other_phone"] = arrPh["ph3"]
//            contactArray["contact_description"] = contact.description
//            contactArray["contact_address"] = contact.address
//            contactArray["contact_city"] = contact.city
//            contactArray["contact_state"] = contact.state
//            contactArray["contact_country"] = contact.country
//            contactArray["contact_zip"] = contact.zip
            print(contactArray)
            arrToImport.append(contactArray)
        }
        
        if arrToImport.count > 0{
            if OBJCOM.isConnectedToNetwork(){
                OBJCOM.setLoader()
                self.importDeviceContacts(contactArray:arrToImport)
            }else{
                OBJCOM.NoInternetConnectionCall()
            }
        }else{
    ...
    func importDeviceContacts(contactArray : [[String : String]]){
        
        if contactArray.count > 0 {
            print("----------------------------------")
            let dictParam = ["userId": userID,
                             "platform":"3",
                             "contact_details":contactArray] as [String : Any]
            ...
            OBJCOM.modalAPICall(Action: "importCrmDevice", param:dictParamTemp as [String : AnyObject],  vcObject: self){
                ...
                if success == "true"{
                    ...
                    alertVC.addAction(PMAlertAction(title: "Remove duplicate contacts", style: .default, action: { () in
                        if OBJCOM.isConnectedToNetwork(){
                            OBJCOM.setLoader()
