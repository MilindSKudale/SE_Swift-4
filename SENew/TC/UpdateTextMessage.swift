...
class UpdateTextMessage: UIViewController, UITextFieldDelegate {
   ...
   // @IBOutlet var lblCampaignName : UILabel!
    ...
    func updateRadioButton(_ sender:UIButton) {
        self.txtInterval.endEditing(true)
        self.editorView.endEditing(true)
    ...
    @IBAction func actionUpdateTextMessage(_ sender: UIButton){
        self.editorView.endEditing(true)
        if validate() == true {
...
extension UpdateTextMessage: RichEditorDelegate {
...
extension UpdateTextMessage: RichEditorToolbarDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension UpdateTextMessage : TagListViewDelegate, UIDocumentPickerDelegate {
    ...
    func designUI(){
        ...
        OBJCOM.modalAPICall(Action: "getTextMessageById", param:dictParamTemp as [String : AnyObject],  vcObject: self) {
            ...
            if success == "true"{
                ...
                self.txtCampaignName.text = result["txtTemplateTitle"] as? String ?? ""
                ...
                if self.isFooterShow == "1" {
                    ...
                    self.footerSwitch.isOn = false
                }
                
                self.isAddSignature = result["txtTemplateAddSignature"] as? String ?? "0"
                if self.isAddSignature == "1"{
                    self.signatureSwitch.isOn = true
                }else{
                    self.signatureSwitch.isOn = false
                }
                ...
                self.isPlainText = result["txtTemplateType"] as? String ?? "1"
                if self.isPlainText == "2" {
                    self.btnPlainText.isSelected = false
                    self.btnCustomText.isSelected = true
                    ...
                    self.btnPlainText.isSelected = true
                    self.btnCustomText.isSelected = false
                ...
                for file in arrAttachments {
                    ...
                    if filePath != "" {
                        ...
                        self.arrAttachFilename.append(filename.last!)
...
//                        self.arrFile.append(url)
                ...
                if self.arrAttachFilename.count > 0 {
                    for i in 0 ..< self.arrAttachFilename.count {
                        self.docTagList.addTag(self.arrAttachFilename[i])
                    }
                }
    ...
    func updateTextMessageAPI(){
        ...
        if strMessageText == "" {
            OBJCOM.setAlert(_title: "", message: "Please insert message content.")
            return
        }
    ...
    func validate() -> Bool {
        ...
        if txtCampaignName.text == "" {
            ...
            return false
        }else if strMessageText == "" || strMessageText == "<br>" {
            OBJCOM.setAlert(_title: "", message: "Please insert text message content.")
            return false
        } else if htmlString == "" {
    ...
    func actionUploadSavedDocs(_ filename:String) {
        ...
        let dictParam = ["userId" : userID,
                         "platform": "3",
                         "txtCampAttachTempId":templateId,
                         "fileOri": filename]
