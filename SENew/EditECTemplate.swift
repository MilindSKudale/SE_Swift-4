...
class EditECTemplate: UIViewController {
    ...
    func designUI(){
        ...
        OBJCOM.modalAPICall(Action: "getEmailTemplateById", param:dictParamTemp as [String : AnyObject],  vcObject: self) {
            ...
            if success == "true"{
                ...
                self.txtRepeatEveryWeeks.text = result["campRepeatWeeks"] as? String ?? "0"
                ...
                if attachements.count > 0 {
                    for file in attachements {
                        ...
                        self.arrAttachFilename.append(filename)
                        self.arrAttachFileId.append(fileId)
                        self.docTagList.addTag(filename)
                    }
                }
                OBJCOM.hideLoader()
            }else{
...
extension EditECTemplate : UITextFieldDelegate {
...
extension EditECTemplate: RichEditorDelegate {
    
    func richEditor(_ editor: RichEditorView, contentDidChange content: String) {
    }
}

extension EditECTemplate: RichEditorToolbarDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension EditECTemplate : TagListViewDelegate, UIDocumentPickerDelegate {
            ...
            case .success(let upload, _, _):
                ...
                upload.responseJSON {
                    ...
                    if let JsonDict = response.result.value as? [String : Any]{
                        ...
                        if success == "true"{
                            OBJCOM.hideLoader()
                            OBJCOM.setAlert(_title: "", message: "File uploaded successfully.")
