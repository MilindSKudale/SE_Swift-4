...
public class AwesomeSpotlightView: UIView {
  ...
  private static let kContinueLabelFont = UIFont.boldSystemFont(ofSize: 15.0)
  private static let kSkipButtonFont = UIFont.boldSystemFont(ofSize: 13.0)
  private static let kSkipButtonLastStepTitle = "Done"
  private static let kContinueButtonLastStepTitle = "Done"
  ...
  public var continueButtonModel = AwesomeTabButton(title: "Next", font: kContinueLabelFont, isEnable: kEnableContinueLabel)
  public var skipButtonModel = AwesomeTabButton(title: "Skip", font: kSkipButtonFont, isEnable: kEnableSkipButton)
  public var skipButtonLastStepTitle = kSkipButtonLastStepTitle
  public var continueButtonLastStepTitle = kContinueButtonLastStepTitle
  ...
  private func setupContinueLabel() {
    ...
    let continueLabelHeight: CGFloat = 50.0
    
    if #available(iOS 11.0, *) {
      continueLabel = UILabel(frame: CGRect(x: 15, y: bounds.size.height - 130, width: bounds.size.width - 30, height: continueLabelHeight))
    } else {
    ...
    continueLabel.layer.cornerRadius = 5.0
    ...
    continueLabel.layer.borderWidth = 1.0
    continueLabel.backgroundColor = .clear //UIColor(patternImage: UIImage(named: "btnBg.png")!)
    continueLabel.clipsToBounds = true
    addSubview(continueLabel)
  }
  
  private func setupSkipSpotlightButton() {
  //  let continueLabelWidth = 0.7 * bounds.size.width
    let skipSpotlightButtonWidth = bounds.size.width
    let skipSpotlightButtonHeight: CGFloat = 50.0
    
    if #available(iOS 11.0, *) {
      skipSpotlightButton = UIButton(frame: CGRect(x: 15, y: bounds.size.height - 130, width: bounds.size.width - 30, height: skipSpotlightButtonHeight))
    } else {
    ...
    skipSpotlightButton.layer.cornerRadius = 5.0
    skipSpotlightButton.layer.borderColor = UIColor.white.cgColor
    skipSpotlightButton.layer.borderWidth = 1.0
...
//        continueLabel.removeFromSuperview()
        continueLabel.text = continueButtonLastStepTitle
      }
    }
  }
