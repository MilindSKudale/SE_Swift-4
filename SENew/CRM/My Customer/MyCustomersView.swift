...
class MyCustomersView: SliderVC {
    ...
    override func viewDidLoad() {
        ...
        actionButton.addItem(title: "", image: #imageLiteral(resourceName: "add_contact")) { item in
            ...
            let vc = storyboard.instantiateViewController(withIdentifier: "idAddCustomerView") as! AddCustomerView
...
extension MyCustomersView : DAOSearchBarDelegate, UITextFieldDelegate {
...
extension MyCustomersView : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction.init(style: UIContextualAction.Style.destructive, title: "Delete", handler: { (action, view, completion) in
            ...
            let alertVC = PMAlertController(title: "", description: "Do you want to delete this Customer?", image: nil, style: .alert)
...
extension MyCustomersView {
    ...
    @IBAction func actionAssignCampaignsToAllRecords(_ sender: UIButton) {
        ...
        if arrSelectedRecords.count > 0 && arrSelectedRecords.count <= 50 {
            ...
            OBJCOM.hideLoader()
            let alertVC = PMAlertController(title: "", description: "You can assign email campaign only 50 customers. Please select customers less than 50.", image: nil, style: .alert)
            
            alertVC.addAction(PMAlertAction(title: "OK", style: .default, action: { () in
                self.dismiss(animated: true, completion: nil)
            }))
        } else{
            OBJCOM.setAlert(_title: "", message: "Please select at least one customer to continue")
            return
        }
...
extension MyCustomersView {
    @IBAction func actionMoreOptions(_ sender:AnyObject) {
        ...
        let item2 = ActionSheetItem(title: "Import Customers CSV", value: 2)
        let item3 = ActionSheetItem(title: "Export Customers", value: 3)
        ...
        let sheet = ActionSheet(items: items) { sheet, item in
            if item.title != "Dismiss"{
                if item == item2 {
                    ...
                    self.createCSV(from: self.prospectCsvArray, crmFlag: "2")
                }else if item == item4 {
                    let storyboard = UIStoryboard(name: "MyCustomers", bundle: nil)
                    let vc = storyboard.instantiateViewController(withIdentifier: "idHelpMyCustomers") as! HelpMyCustomers
                    vc.modalTransitionStyle = .coverVertical
                    vc.modalPresentationStyle = .custom
                    vc.view.backgroundColor = UIColor.darkGray.withAlphaComponent(0.8)
                    self.present(vc, animated: true, completion: nil)
    ...
    func createCSV(from recArray:[Dictionary<String, AnyObject>], crmFlag : String) {
        ...
        do {
            ...
            let fileURL = path.appendingPathComponent("csvCustomers.csv")
    ...
    func exportCSVAPI(crmFlag : String){
        ...
        OBJCOM.modalAPICall(Action: "sendCsvbyEmailCrm", param:dictParam as [String : AnyObject],  vcObject: self) {
            ...
            if success == "true"{
                ...
                let alertVC = PMAlertController(title: "", description: "Customers exported successfully.", image: nil, style: .alert)
