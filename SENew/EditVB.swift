...
class EditVB: UIViewController {
    ...
    @IBOutlet var btnSelectCat : UIButton!
    ...
    func assignDataToFields(_ data: AnyObject){
        ...
        vCategory = data["vbCategory"] as? String ?? ""
        txtDescription.text = data["vboardDescription"] as? String ?? ""
        txtEmotion.text = data["vboardEmotion"] as? String ?? ""
        let category = data["vbCategory"] as? String ?? ""
        self.btnSelectCat.setTitle(category, for: .normal)
        if self.arrVBCategoryTitle.contains(category) {
            let index = self.arrVBCategoryTitle.index(of: category)
            self.vCategory = self.arrVBCategoryId[index!]
        }
        ...
        if arrimgUrl.count > 0 {
            ...
            self.vbAttachId = imgUrl["vbAttachmentId"] as? String ?? ""
    ...
    @IBAction func deleteImage(_ sender: UIButton) {
        ...
        alertVC.addAction(PMAlertAction(title: "Remove", style: .default, action: { () in
            ...
            if OBJCOM.isConnectedToNetwork(){
                OBJCOM.setLoader()
                self.deleteVisionAttachment(attachmentId: self.vbAttachId)
            }else{
                OBJCOM.NoInternetConnectionCall()
            }
        }))
            ...
            for obj in fileData {
                ...
                if let data = try? Data(contentsOf: url!) {
                    ...
                    DispatchQueue.main.async {
                        OBJCOM.setLoader()
                        self.uploadVBImages(data, filename: "vb_\(time).jpg", vbId: self.vbId) {
                            JsonDict in
    ...
    func deleteVisionAttachment(attachmentId:String) {
        ...
        OBJCOM.modalAPICall(Action: "deleteVisionAttachment", param:dictParamTemp as [String : AnyObject],  vcObject: self){
            ...
            if success == "true"{
                ...
                alertVC.addAction(PMAlertAction(title: "OK", style: .default, action: { () in
                    self.imgVision.image = #imageLiteral(resourceName: "no_image")
...
extension EditVB : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func openCamera(){
